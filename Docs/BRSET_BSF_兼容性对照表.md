...
> 更新日期：2026-03-23
> 转换器版本：FilterPresetConverter 1.5
...
| 排除组件(自定义) | ExcludeCustomMod | 152 | ExcludeModValue类型，Strict=true(严格排除) |
...
示例（排除自定义Mod "ParticleOverlay"）：
```json
{
  "Type": 152,
  "Value": {
    "ModName": "ParticleOverlay",
    "Strict": true
  }
}
```
...
| 日期 | 版本 | 更新内容 |
|------|------|----------|
| 2026-03-23 | 1.5 | 同步BSF端排除Mod新特性：新增ExcludeModValue类型，支持严格/部分排除模式，BRSET默认严格排除 |
